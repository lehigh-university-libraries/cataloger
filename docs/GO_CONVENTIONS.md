...
- **Unit tests for providers**: Mock API responses to avoid real API calls
- **Integration tests**: Use test fixtures from `fixtures/` directory
- **Table-driven tests**: Use for evaluation metrics calculations and provider logic
- **Race detection**: Run `go test -race ./...` to catch concurrency issues

## Common Pitfalls to Avoid

- ❌ Don't ignore errors - always wrap with context using `fmt.Errorf("context: %w", err)`
- ❌ Don't log API keys, tokens, or sensitive configuration
- ❌ Don't make assumptions about provider responses - always validate
- ✅ Always validate provider configuration before making API calls
